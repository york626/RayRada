...
- **点「网速」块看流量统计**：**卡片总览**（今天 / 最近 30 天 / 最近 1 年，蓝=下行 橙=上行）+ **堆叠柱状图**（悬停显示当天明细，可切换最近 14 / 30 天，Y 轴自动 KB/MB/GB；**可下拉只看某个应用**）+ **应用流量排行**（Top 10 横向条，可切今天 / 30 天 / 1 年）+ **明细两页**（「按天明细」与「按应用明细（带搜索框）」，都保留一整年）
...
「按应用明细」页（按应用看今天 / 30 天 / 1 年，带搜索框）：

![按应用明细](screenshots/radar-traffic-apps.png)
...
> **按应用**那部分走 ETW（`Microsoft-Windows-Kernel-Network`，与任务管理器「网络」列同源）：统计的是 IP 层字节且**纯 ACK 不产生事件**，所以「按应用合计」通常**略小于**卡片里的网卡总量；回环（127.0.0.1）不计。亚秒级的短命进程（例如跑一下就退出的命令行工具）可能只显示成 `PID 1234`——事件到达时进程已经退出，名字查不到了。
