...
- **点「网速」块看流量统计**：今天（最近 1 天）/ 最近 30 天 / 最近 1 年 的 ↓下行 ↑上行 合计，另有最近 14 天明细
...
- 双击或右键浮窗即开设置窗口；`RayRadar.exe /settings` 可直接启动到设置
...
> 流量统计只统计 **Ray雷达 运行期间**的流量，从首次运行当天开始累计——Windows 自身不提供按天的历史流量（历史上网量在 SRUM 的 ESE 数据库里，读取需管理员且要解析 ESE，本项目不做）。
